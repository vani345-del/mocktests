...
// Use the thunks from your existing mockTestSlice
import {
  fetchAdminMockTests,
  deleteMockTest,
  togglePublish,
...
const ManageMocktests = () => {
  ...
  /* ---------------------- SELECTORS (read from state.mocktest) ---------------------- */
    ...
    <div className="overflow-x-auto bg-white rounded-xl shadow-2xl border border-gray-100/50 mt-6">
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50/70">
          <tr>
            <th className="px-5 py-3 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">Title / ID</th>
            ...
            <th className="px-5 py-3 text-center text-xs font-bold text-gray-600 uppercase tracking-wider">Enrollments</th>
            ...
            return (
              <tr key={test._id} className="hover:bg-blue-50/50 transition">
                <td className="px-5 py-4">
                  ...
                  <p className="text-xs text-gray-400">ID: {test._id.slice(-6)}</p>
                </td>
                ...
                <td className="px-5 py-4 text-center font-bold">{test.attempts?.length || 0}</td>
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <header className="mb-8 border-b pb-4">
        ...
        <div className="flex justify-between items-center mt-4">
          <h2 className="text-3xl font-extrabold text-gray-900">
            Mock Test Registry <span className="text-blue-600">({mocktests.length})</span>
          </h2>

          <button onClick={() => navigate("/admin/tests/add-new-test")} className="flex items-center bg-indigo-600 text-white px-6 py-3 rounded-xl shadow hover:bg-indigo-700">
      ...
      <div className="bg-white p-4 rounded-xl shadow border flex flex-col sm:flex-row gap-4 items-center">
        <label className="flex items-center font-semibold text-gray-700"><FaFilter className="mr-2 text-indigo-500" /> Category:</label>

        {categoriesLoading ? (
          <FaSpinner className="animate-spin text-indigo-500" />
        ) : (
          <select value={selectedCategory} onChange={handleCategoryChange} className="border px-4 py-3 rounded-lg bg-gray-50 focus:ring-indigo-500">
      ...
      {hasError && <div className="mt-10 text-center text-red-500 text-xl">{error}</div>}

      {!isLoading && !hasError && mocktests.length > 0 && <MockTestTable tests={mocktests} />}
